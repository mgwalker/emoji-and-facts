...
const TAG = 'emoji-go-straight-then-turn-then-go-straight-then-turn-etc';

class RightAngleEmojiTakeTwo extends EmojiBase {
  constructor() {
...
div {
  ...
  move() {
    ...
    if (moves >= 50) {
      this.meta.direction += ((oneOrTheOther() ? -1 : 1) * 90) % 360;
      this.meta.moves = 0;
    } else {
      this.meta.moves += 1;
    }
...
customElements.define(TAG, RightAngleEmojiTakeTwo);
